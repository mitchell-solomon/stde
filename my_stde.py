...
class MLP(nnx.Module):
    ...
    def __init__(self, rng: Array, width: int = 128, depth: int = 4):
        ...
        for _ in range(depth):
            ...
            layers.append(
                nnx.Linear(in_features, width, rngs=nnx.Rngs(params=sub))
            )
            in_features = width
        key, out = jax.random.split(key)
        self.layers = tuple(layers)
        self.out = nnx.Linear(in_features, 1, rngs=nnx.Rngs(params=out))
...
def pushforward_jet(
    ...
    primals, series = jet.jet(
        fun,
        (X,),
        (tuple(series_terms),),
        factorial_scaled=factorial_scaled,
    )
    return primals, list(series)
...
class TrainState(nnx.Module):
    def __init__(self, model: MLP, optimizer: nnx.Optimizer):
        self.model = model
        self.optimizer = optimizer


def make_train_state(rng: Array, cfg: Config):
    factorial_scaled = calibrate_factorial_scaled(rng)
    model = MLP(rng, width=cfg.width, depth=cfg.depth)
    tx = optax.adam(cfg.lr)
    optimizer = nnx.Optimizer(model, tx, wrt=nnx.Param)
    state = TrainState(model, optimizer)
...
def train_step(
    ...
    def _loss_fn(model: MLP):
        return loss_fn(model, X_r, X_b, u_b, factorial_scaled, lambda_bc)

    (loss, (loss_r, loss_b)), grads = nnx.value_and_grad(_loss_fn, has_aux=True)(state.model)
